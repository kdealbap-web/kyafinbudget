...
export interface ScheduledPayment {
  ...
  account?: { id: string; name: string; bank_slug?: string | null; bank_name?: string | null } | null;
}
    ...
    try {
      const { data, error } = await this.supabase.client
        ...
        .select(`
          ...
          accounts:account_id (id, name, bank_slug, bank_name)
        `)
      ...
      const mapped = ((data ?? []) as Array<Record<string, unknown>>).map((row) => ({
        ...
        account: Array.isArray(row['accounts'])
          ? (row['accounts'][0] as { id: string; name: string; bank_slug?: string | null; bank_name?: string | null })
          : (row['accounts'] as { id: string; name: string; bank_slug?: string | null; bank_name?: string | null } | null),
      }));
